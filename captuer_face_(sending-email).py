...
def send_email_with_photo(file_name):
    subject = "사진 첨부 테스트"
    body = "PIR 센서 감지로 인해 촬영된 사진을 전송합니다."
    ...
    try:
        ...
        smtp_server.login(SENDER_EMAIL, SENDER_PASSWORD)  # 로그인
...
# history = model.fit(x=X, y=y, epochs=200)
...
if np.argmax(predict) == 0:  # [[1. 0.]]이면 0번 인덱스가 가장 큼
    ...
    ser.write(b'OFF\n')
    print("Sent: OFF")
    send_email_with_photo("captured_photo.jpg")
